...
# Evaluating a dataset of bank client information.
...
# Now with a new database: Boston house prices and related features.
...
# What is the average number of rooms (rm) for the second row of data?

print(d2.iloc[1, 6])

# We will focus on three predictors: per capita crime rate (crim), average no. of rooms (rm),
# and index of accessibility to radial highways.
...
# Use the MinMax scaler to convert the three features so all values are between 0 and 1.
...
# Now use the Standard Scaler to standardize the values.
...
# The -0.0 for the mean implies that the mean for all three standardized variables is negative and very, very
# close to zero. 1.001 for the standard deviations means that the data points are largely clustered around the
# mean and there is not a lot of variability between the values of all three variables.

# Lastly, use the Robust Scaler.
...
# Time for database 3: Supermarket transactions.

d3 = pd.read_excel("C:/Users/skyef/Documents/homeworkdata/SupermarketTransactions.xlsx")

# Practice with making dummy variables.

print(d2['Country'].unique())
d2d = pd.get_dummies(d2, columns=['Country'], drop_first=True)
d2d.head()

# One of the three countries is dropped when making dummies because its value can be implied by the other two dummies.
